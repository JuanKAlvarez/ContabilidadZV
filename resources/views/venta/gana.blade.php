			...
			<div class="row">
				<div class="col-sm-6 col-sm-offset-2">
					...
					{!! Form::open([
						...
						<!-- se Incluye el formulario de gana ventas -->
						@include('forms.gana.ventas.index')
			...
			<div class="  table-responsive">
						<table class="table">
							<tr>
								...
								<th class="text-center">Total del Día</th>
							...
							<tr>
								...
								<td class="text-right"> $ {{ number_format( $registro->liquidar,0) }} </td>
								<td class="text-right"> $ {{ number_format( $registro->ganancia,0) }} </td>
								<td class="text-right"> $ {{ number_format( $registro->perdida,0) }} </td>
								<td class="text-center"> {{$registro->nota}} </td>
								<td class="text-right"> $ {{ number_format( $registro->total_dia,0) }} </td>
					...
					@include('modals.gana.ventas.edit')
					@include('modals.gana.ventas.delete')
...
@section('scripts')
	<script src="{{ asset('/js/ganaVentas.js') }}"></script>
@endsection
